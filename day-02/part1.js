...
function resolve1(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const min = range[0];
    const max = range[1];
    const letter = entry[1][0];
    const password = entry[2].split("");
    let letterFound = 0;
    for (let i = 0; i < password.length; i++) {
      const letterInside = password[i];
      if (letterInside === letter) letterFound += 1;
    }

    if (letterFound >= min && letterFound <= max) counter++;
  }
...
function resolve2(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    const max = range[1];
    const letter = entry[1][0];
    const password = entry[2];
    if (password[min - 1] === letter ^ password[max - 1] === letter) counter++;
  }
