...
function resolve1(groups) {
  let answersCounter = 0;
  for (let i = 0; i < groups.length; i++) {
    const group = groups[i].split("\n");
    let questions = [];
    for (let j = 0; j < group.length; j++) {
      let personalAnswer = group[j];
      if (personalAnswer.length > 1) personalAnswer = personalAnswer.split("");
      if (personalAnswer.length === 1) personalAnswer = [personalAnswer];
      personalAnswer.map((answer) => {
        if (!questions.includes(answer)) questions.push(answer);
      });
    }

    answersCounter += questions.length;
  }
