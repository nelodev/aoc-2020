...
function getAllYesAnswers(answers) {
  const groups = answers.map((inp) => inp.split("\n"));
  let count = 0;

  for (let i = 0; i < groups.length; i++) {
    const group = groups[i];
    const allGroupAnswers = group.join("").split("");
    const answersSet = new Set(allGroupAnswers.join("").split(""));
    const groupAnswers = [...answersSet];
    const allAnswered = groupAnswers.filter(personalAnswer => group.every(answer => answer.includes(personalAnswer)));
    const numberOfQuestionsAllYes = allAnswered.length;
    count += numberOfQuestionsAllYes;
  }

  return count;
}

const answers = parseInput(inputText);
const numberOfAllYes = getAllYesAnswers(answers);
console.log(numberOfAllYes);
